...
describe('Non factored payment reserves', () => {
  ...
  it('Create amount non factored payment - Verify Reserve total', async () => {
    const firstTotal = await steps.reserve.total(clientId);
    const reserve = await steps.reserve.create(
      clientId,
      CreateReserveRequestBuilder.nonFactoredPayment(100),
    );
    const secondTotal = await steps.reserve.total(clientId);

    expect(reserve.reason).toBe(ReserveReason.NonFactoredPayment);
