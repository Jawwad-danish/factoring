...
describe('Transfer from positive reserves', () => {
  ...
  it('Create transfer from positive reserve - Verify Reserve total', async () => {
    const firstTotal = await steps.reserve.total(clientId);
    const reserve = await steps.reserve.create(
      clientId,
      CreateReserveRequestBuilder.transferFromPositive(100),
    );
    const secondTotal = await steps.reserve.total(clientId);

    expect(reserve.reason).toBe(ReserveReason.BalanceTransferFromPositive);
