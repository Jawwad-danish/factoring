...
describe('In full invoice broker payments', () => {
  ...
  it('Create Invoice - Verify Invoice - Purchase Invoice - Pay Invoice - In full - In full invoice', async () => {
    ...
    const { accountsReceivableValue } = await steps.invoice.sendPayment(id);
    await steps.brokerPayment.create({
      invoiceId: id,
      amount: accountsReceivableValue,
    });
    const invoice = await steps.invoice.getOne(id);
    expect(invoice.brokerPaymentStatus).toBe(BrokerPaymentStatus.InFull);
  });

  it('Create Invoice - Verify Invoice - Purchase Invoice - Pay Invoice - Shortpay - Shortpay - In full invoice', async () => {
