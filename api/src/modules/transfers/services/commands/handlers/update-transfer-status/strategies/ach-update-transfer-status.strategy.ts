...
@Injectable()
export class AchUpdateTransferStatusStrategy extends BaseUpdateTransferStatusStrategy {
  private logger: Logger = new Logger(AchUpdateTransferStatusStrategy.name);
  ...
  async update(
    ...
    if (batchTransferData.state !== BatchState.Completed) {
      this.logger.log('ACH transfer not completed. Skipping update');
      return;
    }

    const transfer = batchTransferData.transfers.find((transfer) =>
      [TransferPaymentType.ACH].includes(transfer.paymentType),
    );
    if (!transfer) {
      const message = `No ACH transfer was found on batch transfer ${batchTransferData.externalId}`;
      this.logger.error(message);
      throw new ValidationError('ach-transfer-update-status', message);
    }
